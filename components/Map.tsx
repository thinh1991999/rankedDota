...
const Map = ({ match }: { match: MatchLive }) => {
  const heroes = useAppSelector((state) => state.globalData.heroes);
  const {
    players,
    playbackData: { buildingEvents },
  } = match;
  return (
    <section className="w-full h-full">
      ...
      >
        ...
        {buildingEvents.map((e, idx) => {
          const { positionX, positionY, isRadiant, isAlive, indexId } = e;
          const position = normalizeCoords(positionX, positionY);
          console.log(indexId);
          return (
            <svg
              key={idx}
        ...
        {players.map((player) => {
          const {
            heroId,
            isRadiant,
            playbackData: { positionEvents },
          } = player;
          const { x, y } = positionEvents[0];
          const detailHero = getDetaiHero(heroes, heroId);
          const position = normalizeCoords(x, y);
          return (
            <foreignObject
              key={player.heroId}
              width={24}
              ...
              data-radiant="true"
              data-dire="false"
              className={`${isRadiant ? "filter-green" : "filter-red"}`}
            >
              <span>
                <img
                  width={24}
                  height={24}
                  src={`https://cdn.stratz.com/images/dota2/heroes/${detailHero?.shortName}_icon.png`}
