...
const Region = ({
  ...
  useEffect(() => {
    ...
    _.forEach(newMonths, (month) => {
      ...
      dataSea.push(Math.round(getCheckMatch(SEA).matchCount / 10));
      dataNorthAmerica.push(
        Math.round(getCheckMatch(NORTH_AMERICA).matchCount / 10)
      );
      dataSouthAmerica.push(
        Math.round(getCheckMatch(SOUTH_AMERICA).matchCount / 10)
      );
      dataEurope.push(Math.round(getCheckMatch(EUROPE).matchCount / 10));
    });
    setData({
      labels,
      datasets: [
