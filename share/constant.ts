export const GOLD_ICON =
  "https://cdn.stratz.com/images/dota2/coaching/medal_gold.png";
export const DIRE_ICON = "https://cdn.stratz.com/images/dota2/dire_square.png";
export const RADIANT_ICON =
  "https://cdn.stratz.com/images/dota2/radiant_square.png";
export const COLOR_CHART_RADIANT_BG = "rgba(2, 250, 87, 0.7)";
export const COLOR_CHART_RADIANT_BORDER = "rgba(2, 250, 87, 1)";
export const COLOR_CHART_DIRE_BG = "rgba(250, 2, 21, 0.7)";
export const COLOR_CHART_DIRE_BORDER = "rgba(250, 2, 21, 1)";
export const COLOR_GRAY_STRONG = "rgba(162, 161, 161, 0.5)";
export const COLOR_GRAY_WEAK = "rgba(162, 161, 161, 0.2)";
export const COLOR_GOLD_NW = "rgb(234, 179, 8)";
export const COLOR_EXP = "rgba(255, 255, 255, 0.6)";
export const CLOCK_ICON_HTML = `<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 1024 1024" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64zm0 820c-205.4 0-372-166.6-372-372s166.6-372 372-372 372 166.6 372 372-166.6 372-372 372z"></path><path d="M686.7 638.6L544.1 535.5V288c0-4.4-3.6-8-8-8H488c-4.4 0-8 3.6-8 8v275.4c0 2.6 1.2 5 3.3 6.5l165.4 120.6c3.6 2.6 8.6 1.8 11.2-1.7l28.6-39c2.6-3.7 1.8-8.7-1.8-11.2z"></path></svg>`;
